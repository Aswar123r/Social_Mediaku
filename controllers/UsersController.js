...
class UsersController {
    static async Register (req, res, next) {
        ...
        try {
            ...
            if(validateUsername) return res.status(400).json("Username Already Registered!")
            const validatePhoneNumber = await Models.User.findOne({where : {phone_number  : phone_number}})
            if(validatePhoneNumber) return res.status(400).json("Phone Number Already Registered")
